...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		List<Integer> twoHopNeighbors = null;

		// First, get the neighbors of v
		List<Integer> firstHopNeighbors = getNeighbors(v);

		// Iterate through the list of first hop neighbors to get their neighbors
		for (int i = 0; i < firstHopNeighbors.size(); i++) {
			if (twoHopNeighbors == null) {
				twoHopNeighbors = getNeighbors(firstHopNeighbors.get(i));
			}
			else {
				twoHopNeighbors.addAll(getNeighbors(firstHopNeighbors.get(i)));
			}
		}
		Collections.sort(twoHopNeighbors);
	...
	/**
	 * Calculate the square of a square matrix
	 * @param a
	 * @return a^2
	 */
	private int[][] matrixSquared(int[][] a) {
		int [][] result = new int[a[0].length][a[0].length];
